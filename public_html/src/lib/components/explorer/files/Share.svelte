<script>
    ...
    let shareAction = new Share($authToken)

    let usingPinCode = false,
        pinCode = [];
    ...
    $: {
        ...
        switch (shareAction.share.type) {
            case 1:
                ...
                shareAction.share.downloadCount = null;
                break;
            case 2:
                if (sharingDownloadCount.length === 0) {
                    sharingDownloadCount = 1;
                } else {
                    sharingDownloadCount = parseInt(sharingDownloadCount.toString().replace(/\D/g, ""));
                }

                shareAction.share.downloadCount = sharingDownloadCount;
...
<div class:hidden={!modalOpen} class="w-full h-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto backdrop-blur bg-gray-600/60">
    <div class="mt-7 opacity-100 duration-500 mt-0 ease-out transition-all sm:max-w-[50%] lg:max-w-[30%] sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex justify-center items-center">
        <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7] w-full">
                ...
                <div class="flex justify-between items-center mb-4">
                    <div class="flex items-center h-12">
                        <label class="relative inline-flex items-center cursor-pointer h-max">
                            <input type="checkbox" class="sr-only peer" on:change={() => usingPinCode = !usingPinCode}>
                ...
                <div class="mb-4">
                    <div class="mb-2">
                        ...
                        <select bind:value={sharingType} id="sharing-type" class="border py-3 px-4 pe-9 block w-full border-gray-200 rounded-lg focus:outline-none text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                            ...
                            <option value="2">Set number of downloads</option>
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg dark:bg-slate-700 dark:border-gray-700 mb-2">
                            <div class="w-full flex justify-between items-center gap-x-1">
                                <div class="grow py-2 px-3">
                                    <input class="focus:outline-none w-full p-0 bg-transparent border-0 text-gray-800 focus:ring-0 dark:text-white" min="1" type="text" bind:value="{sharingDownloadCount}">
                                </div>
                                <div class="flex items-center -gap-y-px divide-x divide-gray-200 border-s border-gray-200 dark:divide-gray-700 dark:border-gray-700">
                                    ...
                                    <button on:click={() => sharingDownloadCount++} type="button" class="w-10 h-10 inline-flex justify-center items-center gap-x-2 text-sm font-medium last:rounded-e-lg bg-white text-gray-800 hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
            ...
            <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-gray-700">
                ...
                {#if shareAction.share.state === 'prepare'}
                    <button on:click={shareAction.create()} type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                        Generate link
                    </button>
                {/if}
            </div>
