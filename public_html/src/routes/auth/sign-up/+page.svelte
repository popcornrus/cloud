<script>
    ...
    onMount(() => {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            toast = await user.signUp(username.value, email.value, password.value)
...
<form action="/auth?/login" method="post" class="shadow rounded-3xl min-w-[25%] max-w-[30%] p-10">
    <h3 class="text-2xl dark:text-white font-bold">Sign Up</h3>
    <div class="my-8 grid gap-y-2">
        ...
        <p>By signing up, you confirm that you've read and accepted our <a
    ...
    <div class="flex justify-center flex-wrap gap-y-2">
        {#if !formInProcess}
            ...
            <div class="w-full text-center">
                ...
                <p class="text-gray-400">Already have an account? <a href="/auth/sign-in"
                                                                   class="text-blue-600 underline font-bold">Sign In</a>
                </p>
            </div>
        {:else}
